package com.websthewords.giftlog.phototaker;
...
public class PhotoTaker {
    ...
    //Camera and Crop Intent parameter
    public boolean isPhotoTaken = false;
    public int outputX = 600;
...
//                    Log.e(TAG, "Capture =" + Uri.fromFile(tempFile).toString());
                ...
                case UCrop.REQUEST_CROP:
                    if (mOnFinishListener != null)
                        mOnFinishListener.OnCropFinish(UCrop.getOutput(data));
                    break;
                ...
                case CROP_IMAGE: {
                    ...
                    if (extras != null) {
                        ...
                        if (writed && mOnFinishListener != null)
                            mOnFinishListener.OnCropFinish(mCropUri);
                    }
                }
                break;
                default:
...
//                Toast.makeText(SampleActivity.this, R.string.toast_unexpected_error, Toast.LENGTH_SHORT).show();
    ...
    public boolean doCropImage(Uri uri) {
        UCrop.of(uri, Uri.fromFile(new File(mActivity.getCacheDir(), "Crop.png"))).useSourceImageAspectRatio().start(mActivity);
        return true;
    }

    public boolean doCropImage1(Uri uri) {
        try {
...
//				intent.putExtra("outputY", outputY);
    ...
    public boolean doImageCapture() {
        try {
            Log.e("PhotoTaker", "doImageCapture");
            ...
            mActivity.startActivityForResult(intentCamera, IMAGE_CAPTURE);


//            File dir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM);
//            File temp = new File(dir, mTemp);
//
